...
def get_size(data: Dict):
    sizes = jtr.tree_map(lambda arr: len(arr), data)
...
class Dataset:
    ...
    def create(cls,
        ...
        data = jtr.tree_map(lambda x: np.atleast_3d(x[None])[0], data)  # Make sure all fields has its feature dimension
        
        if freeze:
            jtr.tree_map(lambda x: x.setflags(write=False), data)
    ...
    def get_subset(self, index: np.ndarray) -> Data:
        return jtr.tree_map(lambda x: x[index], self._dict)
    ...
    def __repr__(self):
        key_shapes = jtr.tree_map(lambda x: x.shape, self._dict)
