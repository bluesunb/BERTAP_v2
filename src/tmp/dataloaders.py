...
class Batch(NamedTuple):        # If we need another fields, make a new batch class for that case. Note that this is basically a tuple.
...
def init_batch(batch_size: int, seq_len: int, obs_dim: int, act_dim: int, goal_dim: int) -> Batch:
    return Batch(
        observations=np.ones((batch_size, seq_len, obs_dim), dtype=np.float32),
        actions=np.ones((batch_size, seq_len, act_dim), dtype=np.float32),
        dones_float=np.ones((batch_size, seq_len, 1), dtype=np.float32),
        masks=np.ones((batch_size, seq_len, 1), dtype=np.float32),
        goals=np.ones((batch_size, seq_len, goal_dim), dtype=np.float32)
    )
...
class TrajDataLoader:
    ...
    def sample(self, batch_size: int = 1, starts: np.ndarray = None) -> Data:
        ...
        batch = jax.tree.map(np.atleast_3d, batch)
        return batch
...
class AntDataLoader(TrajDataLoader):
    ...
    def sample(self, batch_size: int = 1, starts: np.ndarray = None) -> Batch:
        """
        ...
        batch = jax.tree.map(lambda arr: arr * (1 - invalid_mask), batch)
        batch_size = batch["observations"].shape[0]
        ...
        return jax.tree.map(np.atleast_3d, batch)
