...
def calc_recon_loss(config: ModelConfig, batch: Batch, recon: Batch):
    ...
    if not config.goal_conditional:
        ...
        diffs['goals'] = jp.zeros_like(diffs['goals'])
    
    goal_obs_pos_loss = jp.concatenate([diffs['goal'], diffs['observations'][..., :2]], axis=-1).mean() * config.pos_weight
...
def train_step(state: TrainState, batch: jp.ndarray, rng: jp.ndarray, config: TotalConfigs, pmap_axis=None):
    ...
    def loss_fn(params):
        out = state(**batch, trian=True, parmas=params, rngs=rngs, mutable=['vq_stats'])
        (recon, vq_info), updates = out
        
        reocn_loss, info = calc_recon_loss(config.model_config, batch, recon)
        loss = reocn_loss + vq_info["vq_loss"]
        
        info.update({"loss": loss, **vq_info})
        return loss, (info, updates)

    state, (updates, info) = state.apply_loss_fn(loss_fn, has_aux=True, pmap_axis=pmap_axis)
...
def eval_step(state: TrainState, batch: jp.ndarray, rng: jp.ndarray, config: TotalConfigs, pmap_axis=None):
    ...
    recon, vq_info = out
    recon_loss, info = calc_recon_loss(config.model_config, batch, recon)
...
def train_vae(state: TrainState,
    ...
    if eval_batch is None:
        eval_batch = sample_batch_fn(pmap=True)
    ...
    for epoch in tqdm(range(n_epochs), desc="Epochs: ", position=0):
        pbar = tqdm(dataloader, desc="Train: ", position=1)
        for step in pbar:
            ...
            info = flax.jax_utils.unreplicate(info)
            loss = info['loss'].item()
        ...
        eval_info = flax.jax_utils.unreplicate(eval_info)

        if log_interval > 0:
            compare_recons(
                logger=logger,
                env=render_env,
                origs=normalizer.denormalize_concat(jax.device_get(eval_batch), keys=denorm_keys, splits=splits),
                recons=normalizer.denormalize_concat(jax.device_get(eval_recon), keys=denorm_keys, splits=splits),
                global_step=global_step,
                quantized=eval_info["env_vq/indices"][:4],
